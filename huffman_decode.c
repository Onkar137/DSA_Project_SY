...
void import_table(FILE *fp_table, unsigned int *freq){
	...
	while((c=fgetc(fp_table))!=EOF){
                freq[i++] = (unsigned char)c; 		//get the values from the .table file to update the huffman treez
	}
	for (i = 0; i < 128; i++)
...
int main(int argc, char* argv[]){
	...
	char file_name[50]={0},temp[50]={0},temp1[50]={0},temp2[50]={0},temp3[50]={0};		//file name
	...
	if( argc == 2 ) {
		...
		scanf("%s",file_name);
		strcpy(temp3,"./encoded/");
		strcat(temp3,file_name);
		strcat(temp3,".huffman");
		const char *file_path = temp3;
		if (access(file_path, F_OK) != -1) {
			printf("%s File exists\n",temp3);
		} else {
			printf("%s File does not exist Please encode again\n",temp3);
		}
		strcat(temp3,".table");
		file_path = temp3;
		if (access(file_path, F_OK) != -1) {
	...
	strcpy(temp2,"./encoded/");
	//import the frequency table and compressed from file and create the huffman tree
	if((fp_huffman = fopen(strcat(temp2,file_name),"r"))==NULL){
		printf("\nERROR: No such file\n");
		return 0;
	}
	strcat(file_name,".table");			//file pointer for .table file
	if((fp_table = fopen(strcat(temp2,".table"),"r"))==NULL){	//open the file stream
	...
	*strstr(file_name,".huffman") = '\0';		//search for .huffman and remove it
	...
	const char *directory_path = "./decoded";

    struct stat info;
    if (stat(directory_path, &info) == 0 && S_ISDIR(info.st_mode)) {
        printf("Directory exists\n");
    } else {
        strcpy(temp,"mkdir decoded");
		system(temp);		//with .decoded at the end
    }
	...
	strcat(temp,"decoded/");				//file with the original name in the .decoded derectory
	// strcat(temp,".decoded/");
	...
	strcpy(temp1,"rm ");
	strcat(temp1,"./encoded/");
	strcat(temp1,file_name);				
	system(strcat(temp1,".huffman"));	
	system(strcat(temp1,".table"));	
